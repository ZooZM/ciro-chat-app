...
class _GroupCallScreenState extends State<GroupCallScreen> {
  ...
  Future<void> _connectToRoom(String url, String token, {bool isVideo = false}) async {
    try {
  ...
  void _onRoomUpdate() { if (mounted) setState(() {}); }
      ...
      listener: (context, state) {
        if (state is CallIdle || state is CallEnded) {
          ...
          _room?.disconnect().then((_) {
            if (mounted) {
              if (canPop) { router.pop(); } else { router.go('/home'); }
            }
          });
        }
      },
  ...
  Widget _buildConnecting() => Center(
    child: Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      Text('call_joining'.tr(), style: const TextStyle(color: Colors.white, fontSize: 16)),
    ]),
  );
  ...
  Widget _buildCallBody() {
    return BlocBuilder<CallCubit, CallState>(
      builder: (context, state) {
        ...
        final isMutedLocally = callActive &&
  ...
  Widget _buildHeader({
    ...
    return Padding(
      padding: EdgeInsets.only(top: 24.resH, bottom: 12.resH, left: 16.resW, right: 16.resW),
      child: Stack(
          ...
          Column(
            ...
            children: [
              ...
              if (isWaiting)
                ...
                Text(
                  'call_participants_count'.tr(namedArgs: {'count': '$participantCount'}),
              ...
              child: GestureDetector(
                ...
                child: Container(
                  ...
                  child: const Row(mainAxisSize: MainAxisSize.min, children: [
                    Icon(Icons.stop_screen_share, color: Colors.white, size: 14),
                    SizedBox(width: 4),
                    Text('Stop', style: TextStyle(color: Colors.white, fontSize: 12)),
                  ]),
                ),
              ),
            ),
  ...
  Widget _buildWaitingCenter() => Center(
    child: Container(
      ...
      child: const Center(
        child: Icon(Icons.group, size: 80, color: _kGreen),
      ),
    ),
  );
      ...
      videoTrack: (!_isCameraDisabled && _room != null)
          ? _room?.localParticipant?.videoTrackPublications
              .where((pub) => pub.source == TrackSource.camera &&
                  pub.track is VideoTrack && !pub.muted)
  ...
  Widget _buildRemoteTile(RemoteParticipant participant, int index) {
    ...
    final isMuted     = participant.audioTrackPublications
        .any((pub) => pub.muted || pub.track == null);
  ...
  Widget _buildControls({required bool isSharing}) {
    return Container(
      padding: EdgeInsets.only(top: 20.resH, bottom: 32.resH, left: 20.resW, right: 20.resW),
      decoration: const BoxDecoration(
        color: _kControlsBg,
          ...
          Wrap(
            ...
            children: [
              ...
              _buildIconBtn(
                ...
                label: _isMicMuted ? 'call_btn_muted'.tr() : 'call_btn_mute'.tr(),
                active: false,
                onTap: () async {
              ...
              BlocBuilder<CallRecordingCubit, CallRecordingState>(
                builder: (context, recState) {
                  ...
                  return _buildIconBtn(
                    ...
                    onTap: () {
                      if (isRecording) {
                        context.read<CallRecordingCubit>().stop(callRoomName: widget.roomId);
                      } else {
