...
class _ContactsScreenState extends State<ContactsScreen> {
  ...
  Future<void> _startPrivateChat(ChatSession targetUser) async {
    ...
    try {
      final roomId = await getIt<ContactsService>().resolvePrivateChat(targetUser.id);
      
      if (mounted) Navigator.pop(context);

      final mutableRoutePayload = ChatSession(
        id: roomId, 
      ...
      if (mounted) context.pushReplacement('/chat_room', extra: mutableRoutePayload);

    } catch (err) {
      if (mounted) Navigator.pop(context);
      
      if (mounted) {
         ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Failed to start chat: $err')));
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('Select Contact', style: AppTypography.headline2.copyWith(color: Colors.black)),
        backgroundColor: Colors.white,
        elevation: 0,
        leading: IconButton(
      ...
      body: BlocBuilder<ChatCubit, ChatState>(
        builder: (context, state) {
             ...
             return Center(
               child: Column(
                 ...
                 children: [
                   ...
                   Text('Syncing Ciro Connect contacts...', style: AppTypography.body1),
                 ],
               ),
             );
          }
          ...
          if (state is ChatContactsSynced) {
            ...
            if (contacts.isEmpty) {
              return Center(
                child: Text('None of your contacts are on Ciro Connect yet.', style: AppTypography.body1.copyWith(color: AppColors.textSecondary))
              );
            }
