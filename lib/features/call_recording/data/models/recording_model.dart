...
class RecordingModel extends Recording {
  const RecordingModel({
    ...
    super.shareStatus,
    super.sharedMessageId,
  });

  factory RecordingModel.fromMap(Map<String, dynamic> map) {
    final shareStatusStr = map['share_status'] as String? ?? 'idle';
    return RecordingModel(
      ...
      shareStatus: ShareStatus.values.firstWhere(
        (e) => e.name == shareStatusStr,
        orElse: () => ShareStatus.idle,
      ),
      sharedMessageId: map['shared_message_id'] as String?,
    );
  }
  ...
  factory RecordingModel.fromEntity(Recording r) {
    return RecordingModel(
      ...
      sharedMessageId: r.sharedMessageId,
    );
  }
}
