...
- Q: How should the app handle message pagination — should the 20-message limit be replaced with WhatsApp-style infinite scroll? → A: Option A — WhatsApp-style infinite scroll. Load initial 30 messages, then lazy-load 30 more on scroll-up.
- Q: How should incoming message deduplication and status regression be prevented during socket reconnects? → A: Option A — Full WhatsApp-style idempotency. Before saving an incoming message, check if `clientMessageId` already exists in SQLite; skip if duplicate. For status updates, query by `clientMessageId` (not MongoDB `id`), and enforce monotonic-only promotion (pending→sent→delivered→read, never backward).
- Q: Should `lastMessageStatus` in the chat list be scoped to the actual latest message, and should ticks only show for sent messages? → A: Option B — Scope to latest message only AND only show ticks when `lastMessageSenderId == currentUserId`. Before updating `rooms.lastMessageStatus`, check if the updated message matches `rooms.lastMessageId`. In the inbox tile, hide tick icons for received messages (WhatsApp behavior).
- Q: How should JIT room creation work — atomic with first message, or keep the two-step delay approach? → A: Option B — Atomic backend. Modify `POST /chat/private/resolve` to accept an optional first message payload (`content`, `clientMessageId`, `type`). The server creates the room, joins the socket channel, and persists+emits the first message in a single transaction. The frontend `_ensureRoom()` removes the 300ms delay.
- Q: Should the app implement WhatsApp-style "Delete for Everyone" message deletion, or keep local-only deletion? → A: Option A — Both modes (WhatsApp-exact). Implement "Delete for Me" (local-only) AND "Delete for Everyone" (socket event `deleteForEveryone`, backend marks message with `isDeleted: true`, all participants see "🚫 This message was deleted"). Time limit: 1 hour after sending.
- Q: Should the Create Poll dialog, Create Event dialog, Poll message bubble, and Event message bubble UIs be refactored to match the WhatsApp reference images in `images_ui/`? → A: Yes. Refactor all 4 UIs to match the reference screenshots layout and structure, but use `AppColors` (not the green WhatsApp palette). Reference images: `create_poll.jpeg`, `create_event.jpeg`, `poll_msg.jpeg`, `event_msg_in_chat.jpeg`.
- Q: Should the ChatInfoScreen "Media, links and documents" header navigate to a dedicated tabbed Media Screen when tapped? → A: Yes. Tapping the header or chevron icon navigates to a new `SharedMediaScreen` with 3 tabs: **Media** (photos, videos, voice notes in grid), **Links**, **Docs**. Layout matches `media_screen.jpeg` reference. Media displays instantly from local SQLite cache (no loading spinner). Footer shows total count (e.g., "626 Photos, 21 Videos"). Tab bar has a "Select" button for multi-select.
- Q: Should voice note waveform data be saved at recording time (sender-side) instead of only being extracted on playback? → A: Yes. When the sender records and sends a voice note, extract the waveform immediately after recording stops and persist it in the message's `metadata.waveformSamples`. Include it in the socket payload so the receiver gets it on first delivery. The receiver stores it in SQLite and never needs to call `extractWaveformData()` — it renders directly from cached data. This eliminates re-extraction on every widget rebuild.
- Q: Should media (images/videos) in chat bubbles be displayed exactly like WhatsApp? → A: Yes. Reference video: `images_ui/display_media_inside_chatroom.mp4`. Key patterns: (1) Media bubbles with ~16px rounded corners, timestamp + delivery ticks overlaid directly on the media (bottom-right corner, semi-transparent dark background behind text). (2) Video bubbles show centered play-button overlay (white triangle in semi-transparent circle) + duration label on thumbnail. (3) Full-screen viewer: solid black background, sender name + date header, share/star/delete action icons, Reply button at bottom. (4) Voice notes in viewer show white waveform with progress bar. All colors use `AppColors`.
...
- What happens when a user tries "Delete for Everyone" after the 1-hour time limit? (Behavior: The option is hidden/disabled; only "Delete for Me" is available).
- What happens when a "Delete for Everyone" event arrives for a message the recipient has already forwarded or saved?
...
- **FR-018**: System MUST implement WhatsApp-style infinite scroll pagination for chat messages. Initial load fetches 30 messages (newest first). Scrolling up triggers lazy-loading of the next 30 messages. `getRoomMessages()` and `watchRoomMessages()` must accept an `offset` parameter. The `_dispatchRecentChatsUpdate()` room list limit of 20 must also be reviewed.
- **FR-019**: System MUST implement WhatsApp-style message idempotency. Incoming messages are deduplicated by `clientMessageId` — if a message with the same `clientMessageId` already exists in SQLite, the insert is skipped. Message status updates MUST query by `clientMessageId` (not MongoDB `_id`). Status transitions MUST be monotonic-only: pending(0)→sent(1)→delivered(2)→read(3). A status update with a lower rank than the current status MUST be silently ignored.
- **FR-020**: System MUST scope `rooms.lastMessageStatus` updates to the actual latest message only. Before updating `rooms.lastMessageStatus` in `updateMessageStatus()`, verify that the message being updated matches `rooms.lastMessageId`. In the chat list tile (`ChatTileWidget`), tick icons (single grey, double grey, double blue) MUST only be displayed when `lastMessageSenderId == currentUserId`. For received messages, no tick icon is shown (WhatsApp behavior).
- **FR-021**: System MUST implement atomic JIT room creation. The backend `POST /chat/private/resolve` endpoint MUST accept an optional first message payload (`content`, `clientMessageId`, `type`, `fileUrl`, `metadata`). When provided, the server creates the room, joins the caller's socket to the room channel, persists the first message, and emits it to the recipient — all in a single transaction. The frontend `_ensureRoom()` MUST remove the `Future.delayed(300ms)` hack and instead use the response to confirm both room creation and first message delivery. The endpoint returns `{ roomId, message? }` where `message` contains the server-assigned `_id` and `createdAt`.
- **FR-022**: System MUST implement WhatsApp-style dual-mode message deletion. **Delete for Me**: removes the message from the local SQLite database only (existing behavior, add confirmation dialog). **Delete for Everyone**: emits a `deleteForEveryone` socket event with `{ chatRoomId, clientMessageId }`. The backend sets `isDeleted: true` on the message document and broadcasts a `messageDeleted` event to all room participants. Recipients replace the message content with "🚫 This message was deleted" and clear any associated media. The "Delete for Everyone" option is only available to the message sender and only within 1 hour of sending. The `Message` entity must add an `isDeleted` boolean field. The `messages` SQLite table must add an `is_deleted INTEGER DEFAULT 0` column.
- **FR-023**: System MUST refactor the Create Poll dialog, Create Event dialog, Poll message bubble, and Event message bubble UIs to match the WhatsApp reference images in `images_ui/`. All colors MUST use `AppColors` tokens (not WhatsApp's green palette). Create Poll: dark modal with QUESTION section, OPTIONS with drag-reorder handles, "Allow multiple answers" toggle, Cancel/Send actions. Create Event: dark modal with event name, description (2048 char limit), start/end date-time pickers, "Include end time" toggle, optional location field, reminder dropdown, "Allow guests" toggle, Cancel/Send actions. Poll bubble: question title, "Select one or more" hint, radio/checkbox options with vote counts and progress bars, timestamp + ticks footer, "View votes" action button. Event bubble: calendar icon, event title, date range, description, "Join call" and "Add to calendar" action buttons.
- **FR-024**: System MUST implement a dedicated `SharedMediaScreen` accessible from `ChatInfoScreen`. Tapping the "Media, links and documents" header navigates to this screen. The screen has a top tab bar with 3 tabs: **Media** (grid of photos, videos, and voice notes from the room), **Links** (list of link-type messages), **Docs** (list of document/file messages). The tab bar includes a "Select" button for multi-select mode. Media loads instantly from SQLite cache without a loading spinner. A footer at the bottom shows the total count summary (e.g., "626 Photos, 21 Videos"). Voice notes appear as waveform thumbnails with duration in the grid. Layout matches `images_ui/media_screen.jpeg` reference, styled with `AppColors`.
- **FR-025**: System MUST optimize voice note waveform handling for WhatsApp-style instant rendering. **Sender side**: After recording stops, extract waveform data (`50 samples`) immediately and persist it in the message's `metadata.waveformSamples` before upload. Include `waveformSamples` in the socket `sendMessage` payload so the receiver receives it on first delivery. **Receiver side**: When a voice note message arrives via socket with `metadata.waveformSamples`, store it in SQLite. When the `_VoiceBubble` widget builds, read cached waveform data from `metadata.waveformSamples` and render directly — do NOT call `extractWaveformData()` or `preparePlayer(shouldExtractWaveform: true)`. The player should only be prepared with `shouldExtractWaveform: false`. Waveform extraction MUST happen at most once per voice note (at recording time), never on playback.
- **FR-026**: System MUST refactor image and video chat bubbles to match WhatsApp-exact media display. Reference: `images_ui/display_media_inside_chatroom.mp4`. Requirements: (1) **Media bubbles**: ~16px rounded corners, timestamp + delivery tick icons overlaid directly ON the media image/thumbnail at bottom-right with a semi-transparent dark gradient behind the text (not below the media). (2) **Video bubbles**: centered play-button overlay (white triangle inside semi-transparent dark circle) + duration label (e.g., "0:14") on the bottom-left of the thumbnail. (3) **Full-screen MediaGalleryViewer**: solid black background, header with sender name + date/time, top-right action icons (share, star, delete), bottom reply button. (4) **Voice notes in viewer**: white waveform centered, playback progress bar at top. All colors use `AppColors` tokens.
...
- **SC-018**: Chat room loads initial 30 messages. Scrolling up lazy-loads 30 more without data loss or scroll position jump.
- **SC-019**: Simulating a socket reconnect that replays already-received messages produces zero duplicate rows in SQLite. Message status never regresses (e.g., `read` → `delivered`).
- **SC-020**: Inbox tick icons (single grey, double grey, double blue) only appear when `lastMessageSenderId == currentUserId`. Received messages show no ticks.
- **SC-021**: First message in a new P2P chat arrives without any `Future.delayed` hack — atomic resolve delivers room creation + message in a single request.
- **SC-022**: "Delete for Everyone" within 1 hour replaces message content with "🚫 This message was deleted" for all participants. Option is hidden after 1 hour.
- **SC-023**: Create Poll dialog, Create Event dialog, Poll bubble, and Event bubble UIs match WhatsApp reference images with `AppColors` palette — zero hardcoded colors.
- **SC-024**: `SharedMediaScreen` loads from `ChatInfoScreen` tap with 3 functional tabs (Media/Links/Docs). Media renders instantly from SQLite — no loading spinner.
- **SC-025**: Voice note waveform extraction happens exactly once (at recording time). Receiver renders waveform from `metadata.waveformSamples` — `extractWaveformData()` is never called on playback.
- **SC-026**: Image/video bubbles display timestamp + ticks overlaid ON the media (not below). Video bubbles show centered play button + duration label. Full-screen viewer has black background with action icons.
