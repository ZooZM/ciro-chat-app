...
## R20. For You two-leg feed merge + packed cursor (v4 — FR-076)

**Finding**: the v1 feed cursor is a single opaque `lastId` over the global recency leg (catalog-looping, R4); injecting followee reposts adds a second, independently-ordered leg (repost `createdAt` desc via the `{reposterId, createdAt}` index over `reposterId ∈ followees(viewer) ∪ {viewer}`). The two legs cannot share one cursor position.

**Decision**:
- **Packed cursor**: the opaque `cursor` becomes a base64-encoded JSON `{ g: <global-leg lastId>, r: <repost-leg last repost createdAt+id> }`; absence of a key = leg exhausted (repost leg) or fresh start. Endpoint 1's contract is unchanged for clients — the cursor was always opaque.
- **Per-page merge**: the service fetches up to `limit` candidates from each leg, interleaves repost items by repost recency into the global stream (simple deterministic zip — e.g. repost items first-fit by recency, no engagement ranking per the spec assumption), trims to `limit`, and advances each leg's cursor by what was actually consumed.
- **Dedup**: within the merged page, one instance per reel — the repost-attributed instance wins over the organic one; multiple followed reposters of the same reel collapse to the most recent (attributed in `repostedBy`). Cross-page dedup is backstopped client-side: `ReelsFeedBloc` drops already-loaded reel ids during pagination (verify at implementation time that the v1 looping feed already tolerates repeats — the loop cycles the catalog, so repeats are by design there; the bloc-level guard must therefore apply only within a repost-injection window, or key items by feed-index rather than reel id — resolve when the bloc delta is written; SC-022 is the acceptance bar).
- **Filters**: both legs compose `visibilityFilter` + creator-edge `blockSet`; the repost leg additionally drops items whose reposter is in the viewer's mutual block set (FR-078) and re-checks that the reposter is still followed at query time (unfollow removes injection organically).
- **Status writer amendment**: R17/R18's "the worker is the only writer of `status`" is superseded by the data-model v4 three-writer rule (worker / report service / admin endpoint — each a guarded `findOneAndUpdate` with a status precondition).

**Alternatives considered**: materialized per-follower feed (fan-out-on-write of reposts) — rejected: write amplification and a new collection for a v1 social mechanic; the read-side two-leg merge rides existing indexes. Aggregation-pipeline `$unionWith` single query — rejected: harder to cursor correctly across legs and to unit-test; two indexed queries + in-memory merge of ≤2×limit docs is simpler and bounded. Client-side merge (two endpoints) — rejected: duplicates dedup/block logic on every client and violates the server-side-only filtering rule (FR-061 posture).

## R21. In-app camera capture (v5 — FR-079/FR-080)

**Decision**: the official **`camera`** package (flutter/packages, `^0.11.x` — exact pin at implementation) hosting a fully custom overlay: `CameraPreview` full-screen, red record button (single continuous clip — tap start/stop, clarified), gallery thumbnail (reuses `image_picker.pickVideo`), flip + flash top-right, `Video | 15s | 30s | 60s` selector. Controller lifecycle: `WidgetsBindingObserver` — on `paused`/`inactive` an active recording is stopped safely (segment ≥1 s proceeds to the trimmer per the interrupted-recording edge case, shorter is discarded with a notice) and the controller is disposed; re-created on `resumed`. Flip re-creates the controller for the other `CameraDescription` (flash control hidden on front). Flash = torch mode while recording. The duration cap drives a progress ring and a timer that calls `stopVideoRecording()` at the cap (SC-025 ±0.5 s); the selector is disabled while recording. Pre-flight permissions via **`permission_handler`** (already a dependency, ^12): camera + microphone requested on first entry; permanent denial renders the FR-079 explanation state with an "open settings" action — never a black preview.

**Alternatives considered**: `camerawesome` — rejected: opinionated built-in UI fights the strict reference layout, larger binary, slower to pin down. `image_picker`'s system camera (v3 approach) — rejected: cannot host the custom overlay/selector at all; this is the entire point of v5. Platform-channel native capture — rejected: 2× per-platform custom code (same reasoning as the v3 trimmer decision).

## R22. Safe-path handoff + trimmer parameterization (v5 — FR-081)

**Decision**: every captured or picked video is **copied to `<appDocumentsDir>/reels_tmp/<uuid>.mp4` before entering the trimmer** — mitigates the confirmed `video_editor` 3.0.0 iOS failure (OSStatus −17913) on paths containing spaces (e.g. picker cache paths under localized folder names); the `reels_tmp` directory is purged when the creation flow exits (post, back-out, or abandon — FR-081's no-partial-reel rule). `reel_trimmer_screen.dart` changes: the `_maxDuration` const becomes a constructor param (`maxDuration` = the capture-time cap 15s/30s/60s for recordings, 60s for gallery — clarified), and the confirm CTA becomes an explicit **"Next"** (`reels.trim_next` i18n key), keeping the existing ffmpeg export + first-frame thumbnail session unchanged.

**Alternatives considered**: trimming in place at the source path — rejected: reintroduces the space-path bug. Skipping the trimmer when a recording already equals the cap — rejected: the spec now mandates the trimmer for every source (FR-081, supersession of FR-060a's conditional rule).

## R23. Mention autocomplete overlay (v5 — FR-083)

**Decision**: a zero-dependency custom widget — the description `TextField` wrapped in `CompositedTransformTarget`; an `OverlayPortal` + `CompositedTransformFollower` panel anchored **below** the field (revised during implementation: FR-082 places the description field near the *top* of the post-details screen, so anchoring above pushed the panel off-screen; below keeps it on-screen while the field's own text stays fully visible either way). A `TextEditingController` listener extracts the active token via a regex on the text before the cursor (`@([\p{L}\p{N}_]{0,30})$`, unicode-aware); while a token is active the panel lists followed users filtered case-insensitively on username/full name; tapping a row rewrites the token to `@username ` (trailing space) and collapses the panel; the panel also collapses on space, `@` deletion, blur, or route pop. State: a small `MentionSuggestionsCubit` (Cubit + Equatable, constructor-injected repository) that loads the following list once when the post-details screen opens and filters in memory — panel rebuilds are isolated to the overlay leaf (FR-014 spirit; the field itself never rebuilds from suggestion state).

**Alternatives considered**: `flutter_mentions` / `flutter_typeahead` — rejected: unmaintained or poor overlay-anchoring control; the custom widget is ~150 lines and matches constitution II exactly. Per-keystroke server-side search — rejected: needless round-trips for follow-list scale; the endpoint still paginates for outliers (R24).

## R24. Followed-users list endpoint (v5 — FR-084)

**Decision**: `GET /api/reels/me/following` — reels module (the `Follow` schema lives there), declared before `/:id` like every static reels path. Query: `follows` where `followerId = caller`, ordered `createdAt` desc (rides the existing `{followerId, createdAt}` leg noted in R20's repost join), cursor-paginated (`limit` default 50, max 100), hydrating followees to `{ id, username, name, avatarUrl }`. Mutually blocked users are filtered out defensively (a block does not delete the follow relation). Contract: endpoint 31. Flutter: `getFollowingUsers` datasource → repository (`Either<Failure, FollowedUserPage>`), fetched once per post-details visit; failure or an empty list simply disables suggestions (FR-083 — typing is never blocked).

**Alternatives considered**: a users-module endpoint — rejected: the Follow schema and its indexes are reels-owned; cross-module reads would violate the existing module boundary. Embedding the list in the profile DTO — rejected: unbounded payload on an unrelated read path.
