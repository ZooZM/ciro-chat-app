...
public class GetUserMediaImpl {
    ...
    private void getDisplayMedia(final Result result, final MediaStream mediaStream, final Intent mediaProjectionData) {
        // Android 14 (API 34) requires a foreground service of type
        // FOREGROUND_SERVICE_TYPE_MEDIA_PROJECTION to be running before
        // MediaProjectionManager.getMediaProjection() is invoked. Start
        // ScreenCaptureService and only continue once its onBind callback
        // confirms onStartCommand (and therefore startForeground) has run.
        final Intent serviceIntent = new Intent(applicationContext, ScreenCaptureService.class);
        try {
            ...
            Log.e(TAG, "Failed to start ScreenCaptureService", t);
            resultError("getDisplayMedia", "Could not start screen-capture foreground service: " + t.getMessage(), result);
            return;
        }
        final ServiceConnection conn = new ServiceConnection() {
            @Override
            public void onServiceConnected(ComponentName name, IBinder service) {
                try { applicationContext.unbindService(this); } catch (Throwable ignored) {}
                continueGetDisplayMedia(result, mediaStream, mediaProjectionData);
            }
            @Override
            public void onServiceDisconnected(ComponentName name) {}
        };
        if (!applicationContext.bindService(serviceIntent, conn, Context.BIND_AUTO_CREATE)) {
            Log.e(TAG, "bindService(ScreenCaptureService) returned false");
            resultError("getDisplayMedia", "Could not bind to screen-capture foreground service", result);
        }
    }

    private void continueGetDisplayMedia(final Result result, final MediaStream mediaStream, final Intent mediaProjectionData) {
