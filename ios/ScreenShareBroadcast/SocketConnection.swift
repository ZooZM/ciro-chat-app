...
class SocketConnection: NSObject {
    ...
    func open() -> Bool {
        guard FileManager.default.fileExists(atPath: filePath) else {
            os_log(.debug, log: broadcastLogger, "failure: socket file missing at %{public}s", filePath)
            return false
        }
    ...
    func stream(_ aStream: Stream, handle eventCode: Stream.Event) {
        ...
        case .hasBytesAvailable:
            if aStream == inputStream {
                ...
                if n == 0 && aStream.streamStatus == .atEnd {
                    close()
...
private extension SocketConnection {
    ...
    func setupAddress() -> Bool {
        ...
        guard filePath.count < MemoryLayout.size(ofValue: addr.sun_path) else {
            os_log(.debug, log: broadcastLogger, "failure: socket path too long")
            return false
        }
    ...
    func connectSocket() -> Bool {
        ...
        guard status == noErr else {
            os_log(.debug, log: broadcastLogger, "connect failure: %d", status)
            return false
        }
        return true
    }
